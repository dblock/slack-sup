  ...
  module Commands
    class Opt < SlackRubyBot::Commands::Base
      ...
      subscribe_command 'opt' do |client, data, match|
        ...
        if mention
          raise SlackSup::Error, "Sorry, only #{user.team.team_admins_slack_mentions.or} can opt users in and out." unless user.team_admin?
        ...
        if mention
          client.say(channel: data.channel, text: "User #{user.slack_mention} is #{'now ' if expression}opted #{user.opted_in? ? 'into' : 'out of'} S'Up.")
        else
          client.say(channel: data.channel, text: "Hi there #{user.slack_mention}, you're #{'now ' if expression}opted #{user.opted_in? ? 'into' : 'out of'} S'Up.")
        end
