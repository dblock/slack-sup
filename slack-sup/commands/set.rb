    ...
    class Set < SlackRubyBot::Commands::Base
      ...
      class << self
        ...
        def set_day(client, channel, data, user, v = nil)
          if user.channel_admin? && v
            ...
            client.say(channel: data.channel, text: "Team S'Up is now on #{channel.sup_day}.")
          elsif v
            client.say(channel: data.channel, text: "Team S'Up is on #{channel.sup_day}. Only <@#{channel.inviter_id}> or a Slack team admin can change that, sorry.")
          else
            client.say(channel: data.channel, text: "Team S'Up is on #{channel.sup_day}.")
          end
          logger.info "SET: #{channel}, user=#{user.user_name}, sup_day=#{channel.sup_day}."
        rescue ArgumentError
          raise SlackSup::Error, "Day _#{v}_ is invalid, try _Monday_, _Tuesday_, etc. Team S'Up is on #{channel.sup_day}."
        end
        ...
        def set_time(client, channel, data, user, v = nil)
          if user.channel_admin? && v
            ...
            client.say(channel: data.channel, text: "Team S'Up is now after #{channel.sup_time_of_day_s} #{channel.sup_tzone_s}.")
          elsif v
            client.say(channel: data.channel, text: "Team S'Up is after #{channel.sup_time_of_day_s} #{channel.sup_tzone_s}. Only <@#{channel.inviter_id}> or a Slack team admin can change that, sorry.")
          else
            client.say(channel: data.channel, text: "Team S'Up is after #{channel.sup_time_of_day_s} #{channel.sup_tzone_s}.")
          end
          logger.info "SET: #{channel}, user=#{user.user_name}, sup_time_of_day=#{channel.sup_time_of_day_s}."
        rescue StandardError
          raise SlackSup::Error, "Time _#{v}_ is invalid. Team S'Up is after #{channel.reload.sup_time_of_day_s} #{channel.sup_tzone_s}."
        end

        def set_followup_day(client, channel, data, user, v = nil)
          if user.channel_admin? && v
            channel.update_attributes(sup_followup_wday: Date.parse(v).wday)
            client.say(channel: data.channel, text: "Team S'Up followup day is now on #{channel.sup_followup_day}.")
          elsif v
            client.say(channel: data.channel, text: "Team S'Up followup day is on #{channel.sup_followup_day}. Only <@#{channel.inviter_id}> or a Slack team admin can change that, sorry.")
          else
            client.say(channel: data.channel, text: "Team S'Up followup day is on #{channel.sup_followup_day}.")
          end
          logger.info "SET: #{channel}, user=#{user.user_name}, sup_followup_day=#{channel.sup_followup_day}."
        rescue ArgumentError
          raise SlackSup::Error, "Day _#{v}_ is invalid, try _Monday_, _Tuesday_, etc. Team S'Up followup day is on #{channel.sup_followup_day}."
        end

        def set_weeks(client, channel, data, user, v = nil)
          if user.channel_admin? && v
            channel.update_attributes!(sup_every_n_weeks: v.to_i)
            client.say(channel: data.channel, text: "Team S'Up is now every #{channel.sup_every_n_weeks_s}.")
          elsif v
            client.say(channel: data.channel, text: "Team S'Up is every #{channel.sup_every_n_weeks_s}. Only <@#{channel.inviter_id}> or a Slack team admin can change that, sorry.")
          else
            client.say(channel: data.channel, text: "Team S'Up is every #{channel.sup_every_n_weeks_s}.")
          end
          logger.info "SET: #{channel}, user=#{user.user_name}, sup_every_n_weeks=#{channel.sup_every_n_weeks_s}."
        rescue StandardError
          raise SlackSup::Error, "Number _#{v}_ is invalid. Team S'Up is every #{channel.reload.sup_every_n_weeks_s}."
        end

        def set_size(client, channel, data, user, v = nil)
          if user.channel_admin? && v
            channel.update_attributes!(sup_size: v.to_i)
            client.say(channel: data.channel, text: "Team S'Up now connects groups of #{channel.sup_size} people.")
          elsif v
            client.say(channel: data.channel, text: "Team S'Up connects groups of #{channel.sup_size} people. Only <@#{channel.inviter_id}> or a Slack team admin can change that, sorry.")
          else
            client.say(channel: data.channel, text: "Team S'Up connects groups of #{channel.sup_size} people.")
          end
          logger.info "SET: #{channel}, user=#{user.user_name}, sup_size=#{channel.sup_size}."
        rescue StandardError
          raise SlackSup::Error, "Number _#{v}_ is invalid. Team S'Up connects groups of #{channel.reload.sup_size} people."
        end

        def set_odd(client, channel, data, user, v = nil)
          if user.channel_admin? && !v.nil?
            channel.update_attributes!(sup_odd: v.to_b)
            client.say(channel: data.channel, text: "Team S'Up now connects groups of #{channel.sup_odd ? 'max ' : ''}#{channel.sup_size} people.")
          elsif !v.nil?
            client.say(channel: data.channel, text: "Team S'Up connects groups of #{channel.sup_odd ? 'max ' : ''}#{channel.sup_size} people. Only <@#{channel.inviter_id}> or a Slack team admin can change that, sorry.")
          else
            client.say(channel: data.channel, text: "Team S'Up connects groups of #{channel.sup_odd ? 'max ' : ''}#{channel.sup_size} people.")
          end
        ...
        def set_timezone(client, channel, data, user, v = nil)
          if user.channel_admin? && v
            ...
            raise SlackSup::Error, "TimeZone _#{v}_ is invalid, see https://github.com/rails/rails/blob/v#{ActiveSupport.gem_version}/activesupport/lib/active_support/values/time_zone.rb#L30 for a list. Team S'Up timezone is currently #{channel.sup_tzone}." unless timezone

            channel.update_attributes!(sup_tz: timezone.name)
            client.say(channel: data.channel, text: "Team S'Up timezone is now #{channel.sup_tzone}.")
          elsif v
            client.say(channel: data.channel, text: "Team S'Up timezone is #{channel.sup_tzone}. Only <@#{channel.inviter_id}> or a Slack team admin can change that, sorry.")
          else
            client.say(channel: data.channel, text: "Team S'Up timezone is #{channel.sup_tzone}.")
          end
        ...
        def set(client, channel, data, user, k, v)
          case k
          when 'opt' then
        ...
        def unset(client, channel, data, user, k)
          case k
          when 'teamfield' then
        ...
        def rotate(client, channel, data, user, k)
          case k
          when 'apitoken' then
        ...
        def team_data_access_message(user, updated_api = false, updated_token = false)
          if user.channel.api? && user.channel_admin? && user.channel.api_token
            "Team data access via the API is #{updated_api ? 'now ' : nil}on with a#{updated_token ? ' new' : 'n'} access token `#{user.channel.api_token}`."
            ...
            "Team data access via the API is #{updated_api ? 'now ' : nil}#{user.channel.api_s}."
          end
        end
      end
      ...
      user_command 'set' do |client, channel, user, data, match|
        if !match['expression']
          team = client.owner
          message = [
            "Team S'Up connects groups of #{channel.sup_odd ? 'max ' : ''}#{channel.sup_size} people on #{channel.sup_day} after #{channel.sup_time_of_day_s} every #{channel.sup_every_n_weeks_s} in #{channel.sup_tzone}, taking special care to not pair the same people more frequently than every #{channel.sup_recency_s}.",
            "Users are _opted #{channel.opt_in_s}_ by default.",
            "Custom profile team field is _#{channel.team_field_label || 'not set'}_.",
            team_data_access_message(user),
            channel.api_url
          ].compact.join("\n")
          client.say(channel: data.channel, text: message)
          logger.info "SET: #{channel} - #{user.user_name}"
        else
          k, v = parse_expression(match)
          set client, channel, data, user, k, v
        end
      end
      ...
      user_command 'unset' do |client, channel, user, data, match|
        if !match['expression']
          ...
          unset client, channel, data, user, k
        end
      end
      ...
      user_command 'rotate' do |client, channel, user, data, match|
        if !match['expression']
          ...
          k, = parse_expression(match)
          rotate client, channel, data, user, k
        end
      end
    end
  end
